package com.leyou.calendar.Ads;
...
public final class AdManager {
    ...
    // ============== 概率触发（保持你的简洁形态） ==============
    private void showRewardedAdConfirmDialog(Activity activity, @Nullable Runnable afterAdClosed) {
        if (rewardedAd == null) {
        ...
        activity.runOnUiThread(() -> {
            AlertDialog dialog = new AlertDialog.Builder(activity)
                    ...
                    .setPositiveButton("YES", (d, which) -> {
                        d.dismiss();
                        showRewardedAd(activity, afterAdClosed);
                    })
    ...
    public void maybeShowAd(Activity activity, @Nullable Runnable afterAdClosed) {
        if (Math.random() < 0.4) {
            boolean pickRewarded = Math.random() < 0.3;
            if (pickRewarded) {
                showRewardedAdConfirmDialog(activity, afterAdClosed);
            } else {
